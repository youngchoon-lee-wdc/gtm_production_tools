...
CVF_HOME=r"C:\Program Files (x86)\SanDisk\CVF_2.0_x64"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if commit_id == '' or product == '' or device_vendor == '' or device_serial_number == '' or device_capacity == '' or project == '' or cvf_version == '':
        ...
        turnon_testmode_bat = os.path.join(os.path.dirname(_filepath), 'turnoff_test_mode.bat')
        p = Popen([turnon_testmode_bat], stdin=PIPE, stdout=PIPE, stderr=PIPE)
        while True:
            output = p.stdout.readline()
            if output == '' and p.poll() is not None:
                break
        ...
        logger.info("rc:%d" % rc)
        if rc != 0:
            raise Exception("Failed to turn on test mode on the batch file")           
